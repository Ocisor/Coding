...
PIXEL_SIZE = 5
ACCELERATION = 1.0  # increased speed
MOMENTUM = 0.95
DOOR_SIZE = 50
WINDOW_OFFSET = 100
...
class Player:
    ...
    def spawn(self, spec):
        ...
        ent = spec.get('entrance')
        # use previous collision coords if available
        entry = self.game.next_entry_coords
        if entry:
            x0, y0, x1, y1 = entry
        else:
            # default center of gap
            mid = ROOM_SIZE // 2
            if ent in ('Left', 'Right'):
                y0 = mid - PIXEL_SIZE//2
            else:
                y0 = 0
            if ent in ('Up', 'Down'):
        ...
        # clamp spawn
        if ent == 'Left':
            x, y = 0, min(max(y0, 0), ROOM_SIZE-PIXEL_SIZE)
        elif ent == 'Right':
            x, y = ROOM_SIZE-PIXEL_SIZE, min(max(y0, 0), ROOM_SIZE-PIXEL_SIZE)
        elif ent == 'Up':
            x, y = min(max(x0, 0), ROOM_SIZE-PIXEL_SIZE), 0
        elif ent == 'Down':
            x, y = min(max(x0, 0), ROOM_SIZE-PIXEL_SIZE), ROOM_SIZE-PIXEL_SIZE
        else:
    ...
    def update(self):
        ...
        if hit:
            ...
            if spec.get('exit') == hit and idx < len(self.game.specs)-1:
                self.game.change(idx+1); return
            if spec.get('entrance') == hit and idx > 0:
                self.game.change(idx-1); return
            if hit in ('Left', 'Right'): self.vx *= -1.5
            else: self.vy *= -1.5
...
class Game:
    def __init__(self, total=20):
        ...
        dx = margin - min_x if min_x < margin else (screen_w-margin-ROOM_SIZE - max_x if max_x+ROOM_SIZE > screen_w-margin else 0)
        dy = margin - min_y if min_y < margin else (screen_h-margin-ROOM_SIZE - max_y if max_y+ROOM_SIZE > screen_h-margin else 0)
    ...
    @property
    def current_panel(self): return self.panels[1]
